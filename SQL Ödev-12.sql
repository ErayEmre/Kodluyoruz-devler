...
--4.sorunun cevabý
SELECT COUNT(payment.customer.id),payment.payment.id,customer.first_name,customer.last_name FROM payment 
JOIN customer ON payment.payment_id=customer.customer_id
GROUP BY payment.payment.id,customer.first_name,customer.last_name
ORDER BY COUNT(payment.customer.id) DESC;